...
export default function Plans({
  ...
  myDeviceId: string;
  partnerName: string;
  onCreate: (title: string, category: string, notes: string, when: string) => void;
  ...
  const submit = () => {
    ...
    onCreate(title.trim(), category, notes.trim(), when.trim());
    setTitle("");
    setNotes("");
    setWhen("");
    setCategory("movie");
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.sub}>Dates, movies, trips — decided together.</Text>

        {plans.length === 0 ? (
          <View style={styles.empty}>
            <Ionicons name="calendar-outline" size={30} color={T.faint} />
            <Text style={styles.emptyText}>
              Nothing planned yet. Propose something you'd love to do together.
            </Text>
          </View>
        ) : (
          plans.map((p) => {
                ...
                <View style={styles.actions}>
                  ...
                  {p.status === "proposed" && iAccepted && (
                    <Text style={styles.waiting}>
                      Waiting for {partnerName || "them"} to say yes…
                    </Text>
                  )}
                  {p.status === "confirmed" && (
                    <Button
                      testID={`plan-complete-${p.id}`}
                      label="Mark done"
                      onPress={() => onComplete(p.id)}
                      style={{ flex: 1, height: 44 }}
                    />
                  )}
                  {p.status === "done" && (
                    <Text style={styles.doneNote}>Saved to your wall ✓</Text>
                  )}
                  {mine && p.status !== "done" && (
                    <Pressable
                      testID={`plan-delete-${p.id}`}
                ...
                {CATEGORIES.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text style={[styles.chipText, { color: on ? "#1A1207" : T.muted }]}>
                        {c.l}
                      </Text>
                    </Pressable>
                  );
...
const styles = StyleSheet.create({
  ...
  empty: { alignItems: "center", paddingVertical: 60, paddingHorizontal: 30, gap: 14 },
  ...
  sheetTitle: { color: T.text, fontFamily: SERIF, fontSize: 28, marginBottom: 8 },
